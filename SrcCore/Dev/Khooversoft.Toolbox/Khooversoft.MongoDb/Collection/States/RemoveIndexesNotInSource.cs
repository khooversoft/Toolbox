...
namespace Khooversoft.MongoDb.Collection.States
{
    public class RemoveIndexesNotInSource : IStateItem
    {
        ...
        public async Task<bool> Set(IWorkContext context)
        {
            ...
            {
                context.EventLog.Info(context, $"No indexes for collection {Parent.Model.CollectionName}");
                return true;
            }
            ...
            foreach (var name in exceptList)
            {
                ...
                context.EventLog.Info(context, $"Index {name} was dropped in collection {Parent.Model.CollectionName}");
            }
        ...
        public async Task<bool> Test(IWorkContext context)
        {
            ...
            context.EventLog.Info(context, $"Indexes {string.Join(", ", exceptList)} are not in source for collection {Parent.Model.CollectionName}");
            return false;
        }
