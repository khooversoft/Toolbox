...
namespace Khooversoft.MongoDb.Collection.States
{
    internal class CreateCollectionState : IStateItem
    {
        ...
        public async Task<bool> Test(IWorkContext context)
        {
            ...
            if (detail == null)
            {
                context.EventLog.Info(context, $"Collection {Parent.Model.CollectionName}, does not exists");
                return false;
            }
            ...
            if (cappedModel != null)
            {
                ...
                context.EventLog.Info(context, $"Collection {Parent.Model.CollectionName} is capped, isEqual={status}");
            }

            context.EventLog.Info(context, $"Collection {Parent.Model.CollectionName}, exists");
            return true;
        }
    }
}
