...
namespace Khooversoft.Toolbox.Actor
{
    ...
    public class ActorTypeManager
    {
        ...
        public ActorTypeManager Register<T>(IWorkContext context, Func<IWorkContext, ActorKey, IActorManager, T> createImplementation) where T : IActor
        {
            ...
            IActor create(IWorkContext c, ActorKey x, IActorManager m) => createImplementation(c, x, m);

            _actorRegistration.AddOrUpdate(
                typeof(T),
                x => new ActorTypeRegistration(x, create),
                (x, _) => new ActorTypeRegistration(x, create));

            context.EventLog.Verbose(context.WithTag(_tag), $"lambda registered for type:{typeof(T)}");
            return this;
        }
        ...
        public ActorTypeManager Register(IWorkContext context, ActorTypeRegistration actorTypeRegistration)
        {
            ...
            _actorRegistration.AddOrUpdate(actorTypeRegistration.InterfaceType, _ => actorTypeRegistration, (_, __) => actorTypeRegistration);

            context.EventLog.Verbose(context.WithTag(_tag), $"lambda registered for type:{actorTypeRegistration.InterfaceType.Name}");
            return this;
        }
        ...
        public T Create<T>(IWorkContext context, ActorKey actorKey, IActorManager manager) where T : IActor
        {
            ...
            if (!_actorRegistration.TryGetValue(actorType, out ActorTypeRegistration typeRegistration))
            {
                ...
                context.EventLog.Error(context.WithTag(_tag), "create failure", ex);
                throw ex;
            }
