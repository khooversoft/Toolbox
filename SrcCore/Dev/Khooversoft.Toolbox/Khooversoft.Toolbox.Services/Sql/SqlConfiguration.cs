        ...
        public bool TestConnection(IWorkContext context)
        {
            ...
            try
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    context.EventLog.Verbose(context, $"Database connect test passed - connection string: {ConnectionString}");
                    return true;
                }
            }
            catch (Exception ex)
            {
                context.EventLog.Error(context, $"Cannot open database - connection string: {ConnectionString}", ex);
                return false;
            }
        }
    }
