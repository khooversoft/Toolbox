...
namespace Khooversoft.Toolbox
{
    public class ProcessRunning : IStateItem
    {
        ...
        public Task<bool> Test(IWorkContext context)
        {
            ...
            //context.EventLog.Info(context, $"Test result: {result}", new { Name = Name });
        ...
        private bool AssertFileExists(IWorkContext context)
        {
            ...
            if (!File.Exists(ExeFullFile))
            {
                ...
                context.EventLog.Error(context, msg);
        ...
        private void OutputToLogging(IWorkContext context, string value)
        {
            if (value.IsNotEmpty())
            {
                context.EventLog.Verbose(context, $"Process {Name} logging: {value}");
            }
        }
    }
}
