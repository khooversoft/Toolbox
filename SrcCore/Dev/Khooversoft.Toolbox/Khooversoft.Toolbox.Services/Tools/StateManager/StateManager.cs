...
namespace Khooversoft.Toolbox
{
    public class StateManager : IStateManager
    {
        ...
        private readonly Tag _tag = new Tag(nameof(StateManager));
            ...
            try
            {
                ...
                foreach (var item in StateItems)
                {
                    ...
                    if (!result)
                    {
                        ...
                        context.CancellationToken.ThrowIfCancellationRequested();
                        context.EventLog.Verbose(context, $"Executed state plan 'Set' for item {item.Name} with {result} result");

                        if (!result && !item.IgnoreError)
                        {
                            context.EventLog.Verbose(context, $"State plan did not completed successfully, workItemIndex={stateContext.WorkItemIndex}");
                            return stateContext;
                        }
                    }
