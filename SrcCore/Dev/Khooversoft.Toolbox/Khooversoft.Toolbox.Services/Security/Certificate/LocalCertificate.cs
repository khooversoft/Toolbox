    ...
    public class LocalCertificate
    {
        ...
        public X509Certificate2 GetCertificate(IWorkContext context, bool? throwOnNotFound = null)
        {
            ...
            lock (_lock)
            {
                ...
                {
                    context.EventLog.Verbose(context, $"Looking for certificate for {this}");
                    ...
                    {
                        context.EventLog.Error(context, $"Exception: {ex}");
                        _cachedCertificate.Clear();
                        saveException = ex;
                    }
                }

                context.EventLog.Verbose(context, $"{(_cachedCertificate != null ? "Found" : "Not found")} certificate for {this}");
