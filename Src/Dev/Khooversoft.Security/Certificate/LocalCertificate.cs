    ...
    public class LocalCertificate
    {
        ...
        public X509Certificate2 GetCertificate(IWorkContext context, bool? throwOnNotFound = null)
        {
            ...
            lock (_lock)
            {
                ...
                {
                    SecurityEventSource.Log.Verbose(context, $"Looking for certificate for {this}");
                    ...
                    {
                        SecurityEventSource.Log.Warning(context, $"Exception: {ex}");
                        _cachedCertificate.Clear();
                        saveException = ex;
                    }
                }

                SecurityEventSource.Log.Verbose(context, $"{(_cachedCertificate != null ? "Found" : "Not found")} certificate for {this}");
